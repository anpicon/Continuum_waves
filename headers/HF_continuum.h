...
//check orthogonality between continuum and bound orbitals
void orthogonality(vec3x& Pelm,vec3x& Pj,vec2d& Occj,int& Lmax,int& mmax,vec1d &R,double dh,int iRmax,vec1d &rho_1,vec1d &rho_2,vec1d &rho_3)
{
    int socca=Occj.size();
    vec3x overlap(SpH(2*Lmax,2*mmax,2*mmax)+1,vec2x(socca,vec1x(SpH(2*Lmax,2*mmax,2*mmax)+1,0.)));
    vec1d sumj(socca,0.);
    for (int L=0; L<=Lmax; L++) {
            ...
            for (int l=0; l<=Lmax; l++) {
                ...
                for (int m=-M1; m<=M1; m++) {
                    for (int j=0; j<socca; j++) {
                        Yk.zeros();
                        do_Yk(Yk,R,dh,iRmax,rho_1,rho_2,rho_3,0,l,m,l,m,mmax,Pj[j],Pelm[SpH(L,M,mmax)],0);
                        overlap[SpH(L,M,mmax)][j][SpH(l,m,mmax)]=Yk[iRmax-1]*R[iRmax-1];//This is the radial overlapping integral
                        sumj[j]+=norm(Yk[iRmax-1]*R[iRmax-1]);//norm((Occj[j][0]+Occj[j][1])*Yk[iRmax-1]*R[iRmax-1])
                    }
                }
            }
        }//---end M
    ...
    ofstream fp_out;
    fp_out.open("Continuum_waves/orthogonality.txt");
    for (int j=0; j<socca; j++) {
        fp_out << j << " " << sumj[j] << endl;
    }
    fp_out.close();
...
void continuum_wave_1h(vec3x &Pelm,double epsilon,int se,cx_mat &Kmat,int iRj,vec1d &R,vec1d &RadialGrid_P,int Lmax,int mmax,vec2d &Atom_xyz,vec2d& Occj,vec3x &Pj) {
    bool iPrint=true;
...
#pragma omp parallel for schedule(dynamic) default(shared)
    ...
    orthogonality(Pelm,Pj,Occj,Lmax,mmax,R,dh,iRmax,rho_1,rho_2,rho_3);
