...
void continuum_wave_1h(vec3x &Pelm,double epsilon,int se,cx_mat &Kmat,int iRj,vec1d &R,vec1d &RadialGrid_P,int Lmax,int mmax,vec2d &Atom_xyz,vec2i& Occj,vec3x &Pj) {
    ...
    if (iJlm_initial==true) {
        ...
        if (iJlm_read==true) {
            ...
            vec4x Radial_part(2*Lmax+1,vec3x(SpH(Lmax,mmax,mmax)+1,vec2x(SpH(Lmax,mmax,mmax)+1,vec1x(iRmax,0.)))); //Radial_part[k][lj][llj][iR]
            double th_W=1.e-14;
#pragma omp parallel for schedule(dynamic) default(shared)
            for (int k=0; k<=(2*Lmax); k++){
                cx_vec Yk; Yk.zeros(iRmax);//if this vector is out of the loop, it gives a numerical error, I dunno the reason
                            ...
                            for (int mmj=-M4; mmj<=M4; mmj++) { //Loop mmj
                                ...
                                if (abs(Wj_lj_llj_k)>th_W) {
                                    ...
                                    for (int j=0; j<socca; j++) {
                                        Yk.zeros();
                                        do_Yk(Yk,R,dh,iRmax,rho_1,rho_2,rho_3,k,lj,mj,llj,mmj,mmax,Pj[j],Pj[j],1); //do_Yk(Yk,R,k,l1,lm1,l2,m2,P1,P2)
//#pragma simd
                                        for (int iR=0; iR<iRmax; iR++){
                                            //double r1=rho_1[iR];
                                            Radial_part[k][SpH(lj,mj,mmax)][SpH(llj,mmj,mmax)][iR]+=((Occj[j][0]+Occj[j][1])/(rho_1[iR]*rho_1[iR]))*Yk[iR];
                                        }
                                    }//-- End j loop
                    ...
                    for (int ll=0; ll<=Lmax; ll++) {
                        ...
                        for (int mm=-M2; mm<=M2; mm++) { //Loop m and mm
                            ...
                            for (int k=abs(l-ll); k<=l+ll; k++) {
                                ...
                                if (abs(Wj_l_ll_k_m)>th_W && abs(Wj_l_ll_k)>th_W) { //condition that l+ll+k has to be even
                                    ...
                                    for (int lj=0; lj<=Lmax; lj++) { //Loop lj and llj
                                        ...
                                        for (int mj=-M3; mj<=M3; mj++) { //Loop mj
                                            ...
                                            for (int llj=0; llj<=Lmax; llj++) {
                                                ...
                                                if (abs(mmj)<=llj && abs(mmj)<=mmax && abs(Wj_l_ll_k_m)>th_W && abs(Wj_l_ll_k)>th_W){
                                                    double Sq_lj_llj=sqrt((2*lj+1)*(2*llj+1));
                                                    double sign=(isOdd(mj+mq+m)==true ? -1. : 1. );
//#pragma simd
                                                    for (int iR=0; iR<iRmax; iR++)
                                                    {
                                                        Jlm_temp[SpH(l,m,mmax)][SpH(ll,mm,mmax)][iR]+=sign*Sq_l_ll*Sq_lj_llj*Wj_l_ll_k*Wj_lj_llj_k*Wj_l_ll_k_m*Wj_lj_llj_k_mj*Radial_part[k][SpH(lj,mj,mmax)][SpH(llj,mmj,mmax)][iR];
                                                    }
                                                }//fi
                                            } //end llj loop
                                        } //end mj loop
                ...
                for (int ll=0; ll<=Lmax; ll++) {
                    ...
                    for (int k=abs(l-ll); k<=l+ll; k++) {
                        if (isOdd(l+ll+k)==false) { //condition that l+ll+k has to be even
                            double Wj_l_ll_k=gsl_sf_coupling_3j(2*ll,2*k,2*l,0,0,0);
                            for (int lj=0; lj<=Lmax; lj++) { //Loop lj and llj
                                for (int llj=abs(k-lj); llj<=k+lj; llj++) {
                                    if (isOdd(lj+llj+k)==false && llj<=Lmax) { //condition that lj+llj+k has to be even && llj must be lower or equal to Lmax
                                        double Wj_lj_llj_k=gsl_sf_coupling_3j(2*lj,2*llj,2*k,0,0,0);
                                        double Sq_lj_llj=sqrt((2*lj+1)*(2*llj+1));
                                        int M1;
                                        if (l<=mmax) {M1=l;} else {M1=mmax;}
                                        for (int m=-M1; m<=M1; m++) {
                                            ...
                                            for (int mm=-M2; mm<=M2; mm++) { //Loop m and mm
                                                ...
                                                if (abs(Wj_l_ll_k_m)>th_W) {
                                                    ...
                                                    for (int mj=-M3; mj<=M3; mj++) { //Loop mj
                                                        int mmj=mj-mq; //mmj is fixed with mq and mj
                                                        if (abs(mmj)<=llj && abs(mmj)<=mmax) {
                                                            double Wj_lj_llj_k_mj=gsl_sf_coupling_3j(2*lj,2*llj,2*k,-2*mj,2*mmj,2*mq);
                                                            double sign=1.;
                                                            if(isOdd(mj+mq+m)==true) sign=-1.;
                                                            for (int j=0; j<socca; j++) {
                                                                Yk.zeros();
                                                                do_Yk(Yk,R,dh,iRmax,rho_1,rho_2,rho_3,k,lj,mj,llj,mmj,mmax,Pj[j],Pj[j],1); //do_Yk(Yk,R,k,l1,lm1,l2,m2,P1,P2)
#pragma simd
                                                                for (int iR=0; iR<iRmax; iR++) {
                                                                    double r1=rho_1[iR];
                                                                    Jlm_temp[SpH(l,m,mmax)][SpH(ll,mm,mmax)][iR]+=(Occj[j][0]+Occj[j][1])*sign*Sq_l_ll*Sq_lj_llj*Wj_l_ll_k*Wj_lj_llj_k*Wj_l_ll_k_m*Wj_lj_llj_k_mj*Yk[iR]/(r1*r1);
                                                                    //Jlm[SpH(l,m,mmax),SpH(ll,mm,mmax),iR]+=(Occj[j][0]+Occj[j][1])*sign*Sq_l_ll*Sq_lj_llj*Wj_l_ll_k*Wj_lj_llj_k*Wj_l_ll_k_m*Wj_lj_llj_k_mj*Yk(iR)/(r1*r1);
                                                                }
                                                            }//-- End j loop
                                                        }//--End if
                                                    }
 ...
 */
            ...
            /* Deactived saving the Jlm potential, not used in our applications
            ofstream fp_Jlm;
...
//#pragma simd
            ...
            /*Deactived saving the Vlm potential, not used in our applications
            ofstream fp_Vlm;
    ...
    if(iPrint==true) cout<< "\nTime for continuum potentials calculations: " << diff/CLOCKS_PER_SEC/60. << " min\n";
...
#pragma omp parallel for schedule(dynamic) default(shared)
    ...
    if (iPrint==true) cout<< "\nTotal time for continuum diff. eq.: " << diff/CLOCKS_PER_SEC/60. << " min" << endl;
    diff=((float)clock5-(float)clock1);
    if (iPrint==true) cout<< "\nTotal time for calculation: " << diff/CLOCKS_PER_SEC/60. << " min" << endl;
